...
def remove_class_and_related_tags(root, class_id, namespaces):
    """
    Removes a class and any tag that uses the ID of the class from an XMI document.

    :param root: The root of the XML tree.
    :param class_id: The xmi:id of the class to remove.
    :param namespaces: Namespaces used in the XMI file.
    """
    # Find the class element by xmi:id
    print(class_id)
    class_elem = root.find(f".//*[@xmi:id='{class_id}']", namespaces)
    ...
    # Remove any tag that references the class ID
    for elem in root.findall(f".//*[@*='{class_id}']", namespaces):
        if elem != class_elem:  # Avoid removing the class element again
            parent = elem.getparent()
            if parent is not None:
                parent.remove(elem)
                print(f"Removed tag {elem.tag} referencing class ID {class_id}.")

    # Remove the class element
    if class_elem.getparent() is not None:
        class_elem.getparent().remove(class_elem)
        print(f"Class with ID {class_id} has been removed.")
...
def create_modified_xmi_2(root, namespaces):
    ...
    for class_elem in classes:
        ...
        if r < 0.7:  # 70% chance to remove attributes, 30% to remove the entire class
            ...
            remove_elements_with_matching_attribute_value(modified_root, class_elem.attrib[f"{{{namespaces['xmi']}}}id"])
...
def process_folder_to_file(input_folder, output_folder):
    ...
    for xmi_file in xmi_files:
        ...
        modified_root = create_modified_xmi_2(root, namespaces)
...
if __name__ == '__main__':
    inputfolder = "object"
    outputfolder = "output"
    process_folder_to_file(inputfolder, outputfolder)
